...
export default function Favorites() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#fcfcfd] pt-40 pb-20 px-6 text-center">
        <div className="max-w-md mx-auto bg-white p-12 rounded-[3rem] shadow-xl border border-slate-100">
           <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center text-3xl mx-auto mb-8 shadow-inner">
           ...
           <Link to="/login" className="block bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm tracking-wide uppercase hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 active:scale-95">
             Sign In Now
           </Link>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div>
            <div className="flex items-center space-x-3 mb-4">
              ...
              <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em]">Saved for Later</span>
            </div>
          ...
          <div className="bg-white px-6 py-4 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-4">
            <div className="text-right">
              ...
              <p className="text-2xl font-black text-indigo-600 leading-none">{favorites.length}</p>
            </div>
            <div className="w-px h-8 bg-slate-100"></div>
            <FaGraduationCap className="text-slate-300 text-2xl" />
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white rounded-[2rem] h-[500px] animate-pulse border border-slate-100 shadow-sm"></div>
            ))}
          </div>
        ) : error ? (
          ...
          <div className="text-center py-32 bg-white rounded-[3rem] border border-dashed border-slate-200">
            <div className="w-20 h-20 bg-slate-50 text-slate-200 rounded-full flex items-center justify-center text-3xl mx-auto mb-8">
              <FaHeart />
            </div>
            ...
            <Link to="/universities" className="inline-flex items-center space-x-2 bg-indigo-600 text-white px-10 py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 active:scale-95">
