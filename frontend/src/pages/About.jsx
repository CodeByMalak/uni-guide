    ...
    <div className="bg-zinc-50 min-h-screen pt-40 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-32">
          <AboutCard 
            icon={<FaBullseye className="text-zinc-800" />}
            title="Our Mission"
        ...
        <div className="bg-zinc-100/60 border border-zinc-200 rounded-[3rem] p-12 md:p-20 text-slate-800 relative overflow-hidden shadow-xl shadow-zinc-600/5">
...
function AboutCard({ icon, title, description, color }) {
  const bgClass = color === 'teal' ? 'bg-zinc-100 border-zinc-200' : 
                  color === 'cyan' ? 'bg-cyan-50 border-cyan-100' : 'bg-rose-50 border-rose-100';

  return (
    <div className="bg-white p-12 rounded-[2.5rem] border border-zinc-200/60 shadow-xl shadow-zinc-300/20 hover:translate-y-[-10px] transition-all duration-500 group">
      <div className={`w-20 h-20 ${bgClass} rounded-3xl flex items-center justify-center text-3xl mb-10 shadow-xl shadow-zinc-200/50 group-hover:scale-110 transition-transform`}>
        {icon}
      </div>
...
function StatBox({ label, value }) {
  return (
    <div className="bg-white border border-zinc-200 p-8 rounded-[2rem] text-center shadow-lg shadow-zinc-600/5">
       <p className="text-4xl font-black text-slate-900 mb-2">{value}</p>
       <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-800">{label}</p>
    </div>
  );
}
