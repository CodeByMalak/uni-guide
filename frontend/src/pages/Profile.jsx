    ...
    <div className="min-h-screen bg-[#f8fafc] pt-32 pb-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="bg-white rounded-[3rem] border border-slate-200 shadow-2xl shadow-slate-200/40 overflow-hidden mb-12">
          <div className="h-48 bg-gradient-to-r from-zinc-800 via-zinc-600 to-zinc-900 relative">
          ...
          <div className="px-12 pb-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 -mt-20 mb-8">
              <div className="relative group">
                <div className="w-40 h-40 rounded-[2.5rem] bg-white border-8 border-white shadow-2xl flex items-center justify-center text-zinc-800 text-5xl font-black relative z-10">
                  {initials}
                </div>
                <div className="absolute inset-0 bg-zinc-800 rounded-[2.5rem] blur-2xl opacity-20 group-hover:opacity-30 transition-opacity" />
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                <button
                  ...
                  className={`flex items-center gap-3 px-8 py-4 rounded-[1.5rem] font-black text-sm uppercase tracking-widest transition-all ${
                    ...
                    : "bg-zinc-800 text-white hover:bg-zinc-900 shadow-xl shadow-zinc-800/20"
                  }`}
                >
            ...
            {isEditing ? (
              <form onSubmit={handleUpdateProfile} className="max-w-xl grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border-2 border-transparent rounded-[1.2rem] px-6 py-4 font-bold focus:border-zinc-600 focus:bg-white outline-none transition-all"
                    required
                  />
                </div>
                <div className="col-span-1">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-slate-400">
                   <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-zinc-100 text-zinc-800 text-[10px] font-black uppercase tracking-[0.2em]">
          ...
          <div className="lg:w-1/3 space-y-8">
            <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-xl shadow-slate-200/20">
               ...
               <div className="space-y-6">
                 ...
                 <StatItem icon={<FaCommentDots className="text-zinc-600" />} label="Reviews" value={userComments.length} />
            ...
            <div className="bg-zinc-100 border border-zinc-200 rounded-[2.5rem] p-10 text-slate-800 relative overflow-hidden group">
               <div className="absolute top-0 right-0 w-32 h-32 bg-zinc-800/5 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform" />
               ...
               <button onClick={() => navigate('/universities')} className="bg-zinc-800 text-white px-8 py-4 rounded-[1.2rem] font-black text-xs uppercase tracking-widest relative z-10 hover:bg-zinc-900 shadow-lg shadow-zinc-800/10 transition-colors">
                  Browse All
               </button>
            </div>
          </div>
                  ...
                  {['info', 'comments', 'favorites'].map(tab => (
                    <button
                      ...
                      className={`flex-1 py-8 text-[10px] font-black uppercase tracking-[0.3em] transition-all relative ${
                        activeTab === tab ? "text-zinc-800" : "text-slate-400 hover:text-slate-600"
                      }`}
                    >
                      {tab === 'info' ? 'My Information' : tab === 'comments' ? `My Reviews (${userComments.length})` : 'My Favorites'}
                      {activeTab === tab && (
                        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 h-1 bg-zinc-800 rounded-full" />
                      )}
                    </button>
                  ))}
                </div>
                   ...
                   {activeTab === 'info' && (
                     <div className="space-y-10 animate-fadeIn">
                        ...
                        <div className="pt-10 border-t border-slate-50">
                           ...
                           <button className="text-zinc-800 font-bold text-sm hover:underline">Update Password & Security Settings</button>
                        </div>
                     </div>
                   )}
                     ...
                     <div className="animate-fadeIn">
                        {loadingComments ? (
                          ...
                          <div className="text-center py-20">
                            ...
                            <button onClick={() => navigate('/universities')} className="bg-zinc-800 text-white px-10 py-4 rounded-[1.2rem] font-black text-xs uppercase tracking-widest shadow-lg shadow-zinc-800/25">
                               Start Exploring
                            </button>
                          </div>
                        ) : (
                          <div className="space-y-8">
                             {userComments.map(comment => (
                               <div key={comment._id} className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100 hover:bg-white hover:border-zinc-200 hover:shadow-xl hover:shadow-slate-200/50 transition-all group">
                                  <div className="flex items-center justify-between mb-4">
                                     <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl bg-zinc-800 text-white font-black flex items-center justify-center text-lg">
                                           {initials}
                                        </div>
                                        <div>
                                           <h4 className="font-black text-slate-900 group-hover:text-zinc-800 transition-colors">
                     ...
                     <div className="animate-fadeIn">
                        {!user.favorites || user.favorites.length === 0 ? (
                          <div className="text-center py-20">
                            ...
                            <button onClick={() => navigate('/universities')} className="bg-zinc-800 text-white px-10 py-4 rounded-[1.2rem] font-black text-xs uppercase tracking-widest shadow-lg shadow-zinc-800/20">
                               Explore Unis
                            </button>
                          </div>
                        ) : (
                             ...
                             {user.favorites.map(uni => (
                               <div key={uni._id} className="p-6 rounded-[2rem] bg-slate-50 border border-slate-100 flex flex-col gap-4">
                                  ...
                                  <div className="flex items-center justify-between mt-auto">
                                     <span className="text-[10px] font-black text-zinc-800 uppercase tracking-widest">{uni.type}</span>
                                     <button onClick={() => navigate(`/university/${uni._id}`)} className="text-xs font-black text-slate-400 hover:text-zinc-800 transition-colors">Details</button>
                                  </div>
                                </div>
                             ))}
                          </div>
