      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-20">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-zinc-100 text-zinc-800 text-[10px] font-black uppercase tracking-[0.2em] mb-6 border border-zinc-200">
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 mb-8 tracking-tighter leading-tight">
              Explore All <span className="text-zinc-800">KPK Universities</span>
            </h1>
          ...
          <div className="md:w-1/2 lg:w-1/3">
             <div className="bg-white p-8 rounded-[2.5rem] border border-zinc-200/60 shadow-xl shadow-zinc-300/30">
                <p className="text-slate-400 font-black uppercase tracking-widest text-[10px] mb-2">Currently Indexing</p>
                <p className="text-4xl font-black text-slate-900">{universities.length} <span className="text-zinc-800">Unis</span></p>
             </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white rounded-[3rem] h-[500px] animate-pulse border border-zinc-200/50 p-10 flex flex-col gap-6 shadow-xl shadow-zinc-200/50">
                 <div className="h-48 bg-zinc-100/60 rounded-[2rem]" />
                 <div className="h-6 w-3/4 bg-zinc-100/60 rounded-full" />
                 <div className="h-4 w-1/2 bg-zinc-100/60 rounded-full" />
                 <div className="mt-auto h-14 bg-zinc-100/60 rounded-[1.5rem]" />
              </div>
            ))}
          </div>
        ) : error ? (
          <div className="text-center py-24 bg-white rounded-[3rem] border border-red-100 shadow-2xl shadow-red-500/5">
            ...
            <button onClick={() => window.location.reload()} className="bg-zinc-800 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-zinc-800/20 hover:bg-zinc-900 transition-all">
               Try Again
            </button>
          </div>
